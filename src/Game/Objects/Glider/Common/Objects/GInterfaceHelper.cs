...
namespace Glider.Common.Objects
{
    public class GInterfaceHelper
    {
        ...
        public int CursorType => GProcessMemoryManipulator.smethod_11(MemoryOffsetTable.Instance.GetIntOffset(nameof(CursorType)), "cursortype");

        public GCursorItemTypes CursorItemType =>
            (GCursorItemTypes)GProcessMemoryManipulator.smethod_11(MemoryOffsetTable.Instance.GetIntOffset(nameof(CursorItemType)),
                "cursoritemtype");
        ...
        public bool IsKeyFiring(string KeyName)
        {
            ...
            try
            {
                gkey = SpellcastingManager.gclass42_0.Offsets[KeyName];
            }
        ...
        {
            var int_1 = GProcessMemoryManipulator.smethod_11(MemoryOffsetTable.Instance.GetIntOffset("UIParent"), "DumpUI_UIParent");
            var num = 0;
            do
            {
                var int_29 = GProcessMemoryManipulator.smethod_11(int_1 + MemoryOffsetTable.Instance.GetIntOffset("UIName"), "uinamedumpui");
                if (int_29 != 0)
                    goto label_2;
                label_1:
                int_1 = GProcessMemoryManipulator.smethod_11(int_1 + MemoryOffsetTable.Instance.GetIntOffset("UINext"), "CurrentAddress");
                continue;
                label_2:
        ...
        public bool IsKeyPopulated(string KeyName)
        {
            try
            {
                SpellcastingManager.gclass42_0.Offsets[KeyName].FilloutKey(true);
                return !SpellcastingManager.gclass42_0.Offsets[KeyName].IsUndefined;
            }
            catch (KeyNotFoundException ex)
            {
        ...
        public static void DumpPreWorldUIDebug(bool RequireVisible)
        {
            ...
            if (num3 != 0 && num3 == num4)
            {
                ...
                do
                {
                    var int_29_3 = GProcessMemoryManipulator.smethod_11(int_1 + MemoryOffsetTable.Instance.GetIntOffset("UIName"), "uinamedumpui");
                    if (int_29_3 != 0)
                        goto label_3;
                    label_2:
                    int_1 = GProcessMemoryManipulator.smethod_11(int_1 + MemoryOffsetTable.Instance.GetIntOffset("UINext"), "CurrentAddress");
