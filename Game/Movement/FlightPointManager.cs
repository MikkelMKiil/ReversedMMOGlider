...
public class FlightPointManager
{
    ...
    {
        var num = GameMemoryAccess.ReadInt32(MemoryOffsetTable.Instance.GetIntOffset("Language"), "Language");
        int_0 = GameMemoryAccess.ReadInt32(MemoryOffsetTable.Instance.GetIntOffset("UIFlightpointCount"), "fpcount");
        ...
        else
        {
            ...
            {
                var int_29 =
                    GameMemoryAccess.ReadInt32(
                        GameMemoryAccess.ReadInt32((index << 5) + MemoryOffsetTable.Instance.GetIntOffset("UIFlightpointBase"),
                            "fpbase") + 4 * num + MemoryOffsetTable.Instance.GetIntOffset("UIFlightpointName"), "fpname");
                if (int_29 != 0)
                {
                    string_0[index] = GameMemoryAccess.ReadString(int_29, 150, "fpnamebytes");
